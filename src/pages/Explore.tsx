...
const Explore: React.FC = () => {
  ...
  const [recommendedConversions, setRecommendedConversions] = useState<{category: string, fromUnit: string, toUnit: string, count: number}[]>([]);
  ...
  const handleQuickConvert = (record: ConversionRecord) => {
    navigate(`/?category=${record.category}&from=${record.fromUnit}&to=${record.toUnit}&value=${record.fromValue}`);
  };
    ...
    <div className="space-y-6 pb-20 md:pb-0">
      ...
      {recommendedConversions.length > 0 && (
        <div className="bg-muted/20 border border-border rounded-lg p-4 space-y-2">
          ...
          <Link 
            to={`/?category=${recommendedConversions[0].category}&from=${recommendedConversions[0].fromUnit}&to=${recommendedConversions[0].toUnit}`}
