...
// Regular expression to parse compound unit expressions
// Matches expressions like "kg/m²" or "m*s^-2"
const compoundUnitRegex = /([a-zA-Z0-9]+(?:\^[+-]?[0-9]+)?)(?:[\*\/\·]([a-zA-Z0-9]+(?:\^[+-]?[0-9]+)?))?/g;

/**
 * Parse a compound unit expression
 * @param expression Unit expression (e.g., "kg/m²")
 * @returns Parsed components with operations
 */
function parseCompoundExpression(expression: string): { 
  symbol: string;
  operation: 'multiply' | 'divide' | null;
  exponent: number;
}[] {
  const components: { 
    symbol: string;
  ...
  let match;
  let operation: 'multiply' | 'divide' | null = null;
  
  // Reset regex state
  compoundUnitRegex.lastIndex = 0;
  
  while ((match = compoundUnitRegex.exec(expression)) !== null) {
    const [_, unitA, unitB] = match;
    
    // Parse first unit and any exponent
    const unitAParts = unitA.split('^');
    const symbolA = unitAParts[0];
    const exponentA = unitAParts.length > 1 ? parseInt(unitAParts[1], 10) : 1;
    
    components.push({
      symbol: symbolA,
      operation: components.length === 0 ? null : operation,
      exponent: exponentA,
    });
    
    // If we have a second unit, parse it too
    if (unitB) {
      const unitBParts = unitB.split('^');
      const symbolB = unitBParts[0];
      const exponentB = unitBParts.length > 1 ? parseInt(unitBParts[1], 10) : 1;
      
      // Look ahead for operation
      const nextChar = expression.charAt(match.index + match[0].length);
      operation = nextChar === '/' ? 'divide' : 'multiply';
 ...
 * @param category Category data
 * @param symbol Unit symbol to find
 * @returns Unit or undefined if not found
 */
function findUnitBySymbol(category: UnitCategory, symbol: string): Unit | undefined {
  // Check direct units first
  if (category.units) {
  ...
  if (category.subcategories) {
    for (const subcategory of category.subcategories) {
      const unit = subcategory.units.find(u => u.symbol === symbol);
      if (unit) return unit;
    }
  }
...
/**
 * FUTURE: Convert between compound units
...
export async function convertCompound(
  ...
  // This is where we would implement compound unit conversion
  // For complex units like kg/m² to lb/ft²
  
  // For now, throw an error as this is not yet implemented
  throw new Error('Compound unit conversion not implemented yet');
}
